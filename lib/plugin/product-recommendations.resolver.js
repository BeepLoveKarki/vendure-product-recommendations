...
let ProductRecommendationAdminResolver = class ProductRecommendationAdminResolver {
    ...
    async updateCrossSellingProducts(ctx, args) {
        const recommendations = await this.productRecommendationService.findAll(ctx, {
        ...
        const promises = toCreate.map((id) => this.productRecommendationService.create(ctx, {
    ...
    async updateUpSellingProducts(ctx, args) {
        ...
        const toCreate = args.productIds.filter((r) => !recommendationsIds.includes(r));
        await Promise.all([
            toCreate.map((id) => this.productRecommendationService.create(ctx, {
                product: args.productId,
                recommendation: id,
